...
export default function InfoPanel({
  ...
  ticketPatente: (patente: number) => void;
  historialPatente: (patente: number) => void;
  editarPatente: (patente: number) => void;
}) {
  ...
  return (
    <div className="z-100 fixed flex items-center gap-3 p-3 border rounded-md shadow-lg bottom-3 left-3 border-neutral-400/20 bg-background/85 backdrop-blur-sm">
      <div className="flex flex-col aspect-5/4 h-14.5 justify-center items-center">
        <div
          className={`border-x-2 text-lg font-mono ${clase.borde} transition-colors px-2`}
        >
          <p>{clickedIndex + 1}</p>
        </div>
      ...
      <h3
        className={`${
          patente === 0 ? "hidden" : ""
        } transition-colors absolute -top-[1px] left-1.5 -translate-y-full font-medium backdrop-blur-md font-mono min-h-8 min-w-23 flex flex-nowrap justify-center items-center border-x border-t rounded-t-xl w-fit ${clase.fondo} ${clase.texto} ${clase.bordeFondo}`}
      >
      ...
      {patente !== 0 && (
        <div className="pl-3 text-lg h-14.5 gap-1.5 flex flex-col items-baseline border-l border-neutral-400/20">
          ...
          <div className="flex gap-1.5">
            <button onClick={() => ticketPatente(patente || 0)} className="text-xs py-0.5 px-2 rounded-full bg-white/60 hover:bg-white/30 focus-visible:bg-white/30 transition-colors cursor-pointer grow text-black justify-center items-center flex">
            ...
            <button onClick={() => historialPatente(patente || 0)} className="text-xs py-0.5 px-2 rounded-full bg-white/60 hover:bg-white/30 focus-visible:bg-white/30 transition-colors cursor-pointer grow text-black justify-center items-center flex">
            ...
            <button onClick={() => editarPatente(patente || 0)} className="text-xs py-0.5 px-2 rounded-full bg-white/60 hover:bg-white/30 focus-visible:bg-white/30 transition-colors cursor-pointer grow text-black justify-center items-center flex">
